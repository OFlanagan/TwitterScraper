...
while (rows_to_process > 0){
  ...
  if (successful_update){
    query <- paste0("update rtweet_table ", 
                    ...
                    "where status_id in (\'", paste(df$status_id, collapse = "\', \'"), "\');")
    dbSendQuery(con, query)
    
    #query to make the initial row count table
    #'INSERT INTO row_counts(source_table,n) select 'tweet', count(*) from tweet;'
...
print(paste0(Sys.time(),"updating row counts"))
update_row_count_query <- "update row_counts set n = (select count(rtweet_table.*) 
from rtweet_table 
